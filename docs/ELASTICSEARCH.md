...
Generated by the macro, `__es_schema__/1` supports these keys:
...
# Get embedded schema metadata (for root and embedded schemas)
Product.__es_schema__(:embeds)
# => [%{name: :items, schema: OrderItem, cardinality: :many, mode: :nested}, ...]

# Check if this schema is embedded (no index/cultures/primary_key of its own)
OrderItem.__es_schema__(:embedded?)
# => true

# Get analyzer atoms referenced by this schema and its entire embed tree
Product.__es_schema__(:analyzer_refs)
# => [:product_search, :stemmer_it]
```
...
Embedded schemas allow you to declare nested structures (like line items inside an order) as reusable, typed components. They are declared separately with `embedded: true`, then included in a root schema via `embeds_one` or `embeds_many`.

### Declaring an embedded schema
...
A schema with `embedded: true` **must not** declare:
- `:index`, `:cultures`, or `:default_culture` (those belong to the root)
- A `settings` block (analyzers are defined once at the root)
- A `primary_key: true` field (the `_id` is owned by the root document)
- A `facets` slot (facets are root-only)

An embedded schema may reference analyzers by name (e.g., `analyzer: :product_search`) — those analyzers are **resolved and validated by the root schema** that uses the embed, ensuring per-culture coverage across the entire embed tree.

### Embedding schemas into a root

Inside a root schema's `schema` block, use `embeds_one` or `embeds_many`:

```elixir
defmodule MyApp.Search.Order do
  ...
  schema do
    ...
    field(:product_name, :text, analyzer: :product_search, searchable: true)
    field(:total, :float)
    field(:status, :keyword)
    
    # Single nested struct, defaults to nil
    embeds_one(:shipping, MyApp.Search.Address)
    
    # List of structs, defaults to []
    # mode: :nested (default) preserves per-entry correlation for combined filters
...
The `:mode` option determines how Elasticsearch indexes an `embeds_many` array. It has significant implications for filtering semantics:

#### `:object` (default, faster, lower cost)

Embeds are stored as Elasticsearch `object` type. Objects in arrays are **flattened**, meaning ES stores parallel value lists rather than preserving entry-wise correlation:
...
When filtering with `filters: [items: [sku: "A", quantity: {:gte, 5}]]`, the filter matches **if any entry satisfies each condition separately**. Both conditions are true somewhere in the array, even though **no single entry** has both. This is a **false positive**:

```elixir
# With mode: :object
{:ok, page} = Orders.get_paged(filters: [items: [sku: "A", quantity: {:gte, 5}]])
# MATCHES because:
#   - Entry 1 has sku "A" (but qty 1)
#   - Entry 2 has qty 5 (but sku "B")
```

**Use `:object`** when:
- You don't need combined filters across multiple fields of the same entry
- You prefer lower index/query cost
- You use `embeds_one` (single entry, no flattening concern)

#### `:nested` (default for multi-field correlation, higher cost)

Embeds are stored as Elasticsearch `nested` type. Each entry is indexed as an internal hidden document, preserving per-entry correlation:

```elixir
# In ES with nested:
# "items" => [
#   {"sku": "A", "quantity": 1},    # internal doc 1
#   {"sku": "B", "quantity": 5}     # internal doc 2
# ]
```

A combined filter now only matches if **all conditions hold on the same entry**:

```elixir
# With mode: :nested
{:ok, page} = Orders.get_paged(filters: [items: [sku: "A", quantity: {:gte, 5}]])
# Does NOT match o-1 because entry 1 has sku "A" but qty 1 (< 5)
# MATCHES o-2 because entry 1 has sku "A" and qty 7 (>= 5)
```

**Use `:nested`** when:
- You need combined filters to be correlated to the same entry
- The cost of internal documents and `nested` queries is acceptable

### Recursive embedding

Embedded schemas can themselves declare `embeds_one` or `embeds_many`, allowing multi-level nesting:

```elixir
defmodule MyApp.Search.OrderItemSub do
  use Orkestra.ES.Schema, embedded: true
...
defmodule MyApp.Search.OrderItem do
  ...
  schema do
    ...
    embeds_many(:subs, MyApp.Search.OrderItemSub, mode: :nested)
  end
end
...
Round-trip (index and retrieve) faithfully preserves all nesting levels.

### Full-text search in embedded fields

Searchable fields inside embeds participate in full-text search via `get_paged(search: "...")`:

#### Via `mode: :object` embeds

Searchable fields from object embeds are added to the `multi_match` with their **dotted paths**:

```elixir
# Root schema with object embed:
embeds_many(:items, OrderItem, mode: :object)  # OrderItem has field(:name, :text, searchable: true)

# In get_paged:
{:ok, page} = Orders.get_paged(search: "drill")
# The multi_match includes "items.name" and matches any order with "drill" in any item name
```

#### Via `mode: :nested` embeds

When a nested embed has searchable fields, Elasticsearch cannot match them via a root-level `multi_match` (a language limitation). Instead, the search clause automatically becomes:
1. A root-level `multi_match` on root fields only, in a `bool should`
2. A `nested` query with path to the embed, containing a `multi_match` on the embed's searchable fields
3. `minimum_should_match: 1` (match if any clause succeeds)

This applies recursively for nested-within-nested:
...
#   bool {
#     should: [
#       multi_match over root searchable fields,
#       nested(path: items) {
#         multi_match over items.name
#       },
#       nested(path: items.subs) {
#         multi_match over items.subs.note
#       }
#     ]
#   }
```

### Filtering embedded fields

Embedded fields are referenced by name in the `:filters` option. Sub-filters follow the same type-aware rules as root fields:

```elixir
# Keyword/exact matches:
get_paged(filters: [items: [sku: "X"]])

# Ranges:
get_paged(filters: [items: [quantity: {:gte, 2}]])

# Multiple conditions (must all hold on the same entry in nested mode):
get_paged(filters: [items: [sku: "A", quantity: {:gte, 5}]])

# Recursive into deeper embeds (for nested-within-nested):
get_paged(filters: [items: [subs: [code: "warranty"]]])
```

For `mode: :object`, all sub-filters are applied **independently** (possible false positives).
For `mode: :nested`, all sub-filters are combined into **one** `nested` query so they must all match the same entry.

Unknown field inside an embed returns `{:error, {:unknown_filter_field, "items.sku_typo"}}` with the full dotted path.

### Sorting limitations

Sorting on embedded fields (e.g., `sort: [items: :asc]` or `sort: [items: [quantity: :asc]]`) is **not currently supported** and returns `{:error, {:not_sortable, field}}`. This is a current limitation.

### Schema-level inheritance

The root schema's `dynamic: "strict"` setting is **inherited** into all embedded properties. Unknown fields inside embeds are rejected at index time, preventing mapping drift and schema confusion.
